...
gamerule `djcraftOffBeatAttackDamagePercent` 控制合法攻击 Miss 的剩余伤害，默认 0，运行时限制为
...
物品 Profile 可以配置 `beat_cooldown`。未配置时按物品主手攻击速度换算：
...
触发型动作和普通攻击通常使用 `max(0, beat_cooldown - 0.4)` 拍作为实际物品冷却，
0.4 拍是现有宽容量。虚拟拍由当前曲目的显式节拍网格换算为 tick，因此变速谱面中的实际
秒数会随局部节拍间隔变化。
...
前摇、冷却、攻击能耗和使用能耗由服务端数据包 Profile 产生完整快照，并在登录和
...
挥手，并开启持续 2 tick 的攻击窗口：开启时先做零位移检测，之后逐 tick 连续扫掠上一眼睛位置到
当前眼睛位置。窗口内旋转视角不会改变本次攻击方向，镜头也不会被目标吸附。
...
`beat_cooldown` 控制达到满弓强度所需的拍数，而不是松开后的额外冷却。若释放判定缓存
...
`djcraft:laser_crossbow` 是 465 耐久、仅出现在创造栏的弩子类。非 DJ 会话完全使用原版弩的
...
- Hit 时每个目标承受 12 点基础伤害再乘当前节拍类别倍率；不卡拍时改乘 gamerule 百分比。之后
  ...
  最多确认一次连击与奖励；
- 三种内置射线武器均支持 `djcraft:ray_overcharge`（射线过载，最高 IV），每级把每次实际射线
  伤害增加 2；激光弩和魔法弩增加直伤，爆炸弓增加爆炸衰减前的中心伤害；
...
辉光，并在枪口、实体交点与终点显示先扩张、再收缩并渐隐的能量爆发；视觉参数可由客户端资源包覆盖。

#### 魔法弩

`djcraft:magic_crossbow` 复用激光弩的 465 耐久、创造栏获取方式、非 DJ 原版弩行为、96 格服务端
...
`djcraft:explosive_bow` 是 384 耐久、仅在创造栏提供的弓。非 DJ 会话保持原版弓的拉弓、箭矢、
...
- 余额不足时服务端强制停止使用物品；
- 放下盾牌、换手、换物或会话结束会清理活动状态；若整次举盾没有成功招架任何伤害，结束举盾时禁用该盾牌 2 个虚拟拍。
...
|---|---:|---:|---|
| `djcraftOffBeatAttackDamagePercent` | 0 | 0–100 | 合法不卡拍攻击保留的原版最终伤害百分比 |
...
| 物品冷却、切换前摇、攻击/使用能耗 | 数据包 `item_timing` |
