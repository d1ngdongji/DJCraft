...
- 播放独立的换出/换入动画；
- F 键换手按进入物品的冷却拍处理，同时保持两只手动画状态隔离；第一人称渲染以较长的
  `switch_warmup` 作为共享视觉交接时长，并在同一 tick 原子提交主副手缓存，快速反向换手会取消
  尚未提交的整组交接，避免出现一旧一新的幽灵物品组合；
- 空手普通攻击在 DJ 会话中被取消。
...
- 切换前摇 1 拍。按下进行一次攻击判定；合法爆炸弓的判定尝试无论成功或失败都会开始 2 拍冷却。
  Hit 或被 gamerule 放行的 Miss 还会立即扣 6 能量并播放蓄力动画，从实际按下时刻起经过 1.0 个
  连续虚拟节拍后自动发射；0% Miss 不扣能量；
...
- Hit 后立即投掷，不经过原版蓄力；
- 默认消耗 10 能量并损耗 1 点耐久；
- 带忠诚的投射物在 DJ 去程和返程期间无重力；无忠诚的三叉戟从发射起使用原版重力，首次碰撞后
  结束 DJ 飞行并恢复原版碰撞箱和拾取/消失行为，不会逐渐减速后悬停在半空；
...
- 原投掷者死亡时，带忠诚的三叉戟会冻结并等待同 UUID 的复活玩家实体；玩家在同维度复活后，三叉戟
  重新绑定新实体并继续原版忠诚返航。死亡等待期间禁止打回和返程伤害；若玩家直接离线，则在当前位置
  掉落可拾取的三叉戟物品并移除投射物，避免永久滞留。其他会话结束或进入旁观时交回原版忠诚逻辑。
  非 DJ 会话投出的三叉戟完全保持原版行为。
...
- 去程和返程的实体命中由服务端按三叉戟完整碰撞箱与目标在同一 tick 的相对位移进行连续扫掠，
  选择最早接触的合法目标；原投掷者始终从实体命中候选中排除，且首次命中后不再重复查询，避免接触
  已停止的三叉戟时重复受到逐帧伤害。双方高速对向移动并在两个 tick 端点交换位置时仍能命中。
  方块仍先截断去程的中心移动线段。
...
则对玩家位置水平半径 4 格、上下各 8 格内的其他存活实体造成伤害并以 1.0 的竖直速度击飞。水平与
垂直范围独立判定。伤害在 3 格时为 4 点，随后线性增长，在 12 格达到 16 点并封顶。至少一个实体
实际受到伤害时，本次下砸按同一动作序列增加
...
- 服务端复用玩家登录时回报的 verified 内容，准备包只包含尚未验证的曲目；
- 客户端仍以同一服务端哈希对应的 verified 状态直接通过，并以当前本地内容哈希作为时序后备；
- 可分发的缺失/不一致归档串行下载；
- 整批安装后只刷新一次客户端资源；
