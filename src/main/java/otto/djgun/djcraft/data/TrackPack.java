...
public record TrackPack(
    ...
    /**
     * Resolves an event's base definition and applies its supported props
     * overrides. Missing definitions use the normal default definition.
     */
    public BeatDefinition resolveDefinition(BeatEvent event) {
        BeatDefinition definition = event == null ? null : getDefinition(event.type());
        if (definition == null) {
            definition = BeatDefinition.createDefault();
        }
        return event == null ? definition : definition.withOverrides(event.props());
    }
    ...
    public int getPlaybackStartMs() {
        return meta != null ? Math.max(0, meta.playbackStartMs()) : 0;
    }

    /**
     * Returns whether the raw audio playback position has reached the exclusive
     * end configured by {@code meta.total_duration_ms}.
     */
    public boolean hasReachedPlaybackEnd(long rawPlaybackPositionMs) {
        int playbackEndMs = getTotalDurationMs();
        return playbackEndMs > 0 && rawPlaybackPositionMs >= playbackEndMs;
    }
    ...
    public String getSummary() {
        ...
        if (meta != null) {
            ...
            sb.append("  - Playback start: ").append(getPlaybackStartMs()).append("ms\n");
        }
        ...
        if (definitions != null) {
            for (String defName : definitions.keySet()) {
                ...
                sb.append("  - ").append(defName)
                        ...
                        .append(", category=").append(def.category().id())
                        .append(")\n");
            }
        }
