...
public class DJCrosshairRenderer {
    ...
    public static void reset() {
        feedbackStartMs = -1;
        feedbackIsHit = false;
        crosshairAnimStartMs = -1;
        dismissedBeats.clear();
    }
    ...
    /** Starts only the shared center-dot line recoil, without Legacy beat/line feedback state. */
    public static void notifyCenterCrosshairAnimation() {
        crosshairAnimStartMs = System.currentTimeMillis();
    }
    ...
    public static void renderCenterCrosshair(GuiGraphics gui) {
        ...
        long elapsed = crosshairAnimStartMs < 0
                ? Long.MAX_VALUE
                : System.currentTimeMillis() - crosshairAnimStartMs;
        float animOffset = DJCrosshairAnimation.lineOffset(elapsed);
    ...
    public static void render(GuiGraphics gui) {
        ...
        if (combatLine != null) {
            for (int j = 0; j < combatLine.size(); j++) {
                ...
                BeatDefinition def = session.getTrackPack().resolveDefinition(beat);
                int rgb = parseHexColor(def.color() != null ? def.color() : "#FFFFFF");
        ...
        renderJudgmentLine(gui, centerX, judgeY,
                DJDeferredDamageIndicatorState.hasPending(session.getSessionId()));
    }
    ...
    private static void renderJudgmentLine(GuiGraphics gui, int cx, int y, boolean deferredDamagePending) {
        int rgb = deferredDamagePending ? 0xFFD83D : 0xDDEEFF;
        float alpha = deferredDamagePending ? 0.85f : 0.55f;
        if (!deferredDamagePending && feedbackStartMs >= 0) {
