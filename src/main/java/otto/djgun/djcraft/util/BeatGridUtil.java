...
public class BeatGridUtil {
     ...
     * @return 持续时间（毫秒）
     */
    public static long getDurationMs(long currentTimeMs, List<BeatEvent> beats, double beatsToWait) {
        return calculateTargetTime(currentTimeMs, beats, beatsToWait) - currentTimeMs;
    }

    /**
     * 计算经过指定节拍数后的持续时间（游戏刻，向下取整，1 tick = 50 ms）
     *
     * @param currentTimeMs 当前时间（相对于曲目开始）
     * @param beats         节拍列表（必须按时间排序）
     * @param beatsToWait   需要等待的节拍数（可以是小数）
     * @return 持续时间（Ticks）
     */
    public static int getDurationTicks(long currentTimeMs, List<BeatEvent> beats, double beatsToWait) {
        return (int) (getDurationMs(currentTimeMs, beats, beatsToWait) / 50.0);
    }
}
