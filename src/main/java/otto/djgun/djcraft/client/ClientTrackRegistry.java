...
public class ClientTrackRegistry {
    ...
    public void onReceiveServerHashes(Map<String, String> serverHashes) {
        ...
        Map<String, String> localHashes = TrackPackManager.getInstance().getContentHashes();
    ...
    /**
     * 获取服务端所有已知的曲目包ID (无论本地是否有)
     */
    public Set<String> getServerPackIds() {
        return serverHashes.keySet();
    }

    public String getServerHash(String packId) {
        return serverHashes.get(packId);
    }
    ...
    private void reportToServer(Map<String, String> localHashes) {
        if (net.minecraft.client.Minecraft.getInstance().getConnection() == null) {
            return;
        }
        Map<String, String> verifiedHashes = verifiedPackIds.stream()
                .collect(Collectors.toUnmodifiableMap(id -> id, localHashes::get));
        PacketDistributor.sendToServer(new ClientTrackStatusPayload(verifiedHashes));
    }
}
