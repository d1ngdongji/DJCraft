...
public class DJSoundManager {
    ...
    public void playTrack(TrackPack trackPack) {
        playTrack(trackPack, 0L);
    }

    public void playTrack(TrackPack trackPack, long initialPositionMs) {
        playTrack(trackPack, initialPositionMs, 0L);
    }

    public void playTrack(TrackPack trackPack, long initialPositionMs, long estimatedTransitMs) {
        // 停止当前播放
        stopTrack();
        long effectiveInitialPositionMs = Math.max(initialPositionMs, trackPack.getPlaybackStartMs());
        ...
        DJSoundInstance sound = new DJSoundInstance(soundLocation, trackPack.id(), vol,
                effectiveInitialPositionMs, estimatedTransitMs, trackPack.getPlaybackStartMs(),
                trackPack.getTotalDurationMs());
        currentSound = sound;
